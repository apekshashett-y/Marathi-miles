...
import { raigadFortLocations, raigadGraphEdges, raigadFortMetadata } from '../../data/raigadFortData.js';
import {
    optimizeRoute,
...
const RaigadSmartExplorationV2 = ({ onBack }) => {
    ...
    const estimateTravelTime = (fromId, toId) => {
        ...
        const edge = raigadGraphEdges.find(e =>
    ...
    const handleComputeRoute = (forceId = null, deviationStart = null, isReroute = false, callback = null, explicitStrategy = null) => {
        ...
        setTimeout(() => {
            ...
            const context = {
                ...
                locations: raigadFortLocations,
                edges: raigadGraphEdges,
            ...
            if (isReroute) {
                ...
                result = optimizeRoute(raigadFortLocations, raigadGraphEdges, context);
    ...
    const handleAddToRoute = (id) => {
        ...
        trainModel(raigadFortLocations[id], 'positive');
        ...
        handleComputeRoute(id, null, false, (result) => {
            ...
            if (!included) {
                ...
                setRerouteBanner({
                    message: `✔ Included ${raigadFortLocations[id].name} (Soft Inclusion)`,
    ...
    const handleRerouteFromLocation = (targetId) => {
        ...
        setRerouteBanner({
            message: `🔄 Route restarted from ${raigadFortLocations[targetId].name}`,
    ...
    useEffect(() => {
        ...
        const location = raigadFortLocations[currentLocationId];
        if (!location) {
            handleSimulationComplete();
            return;
        }
    ...
    const handleLocationClick = (id) => {
        if (!raigadFortLocations[id]) return;
        ...
        <div className={`smart-exploration-v2 theme-${selectedStrategy}`}>
            <header className="exploration-header">
                <div className="header-left">
                    ...
                    <h1>{raigadFortMetadata.fortName} Smart Map</h1>
                ...
                {extensionModal && (
                    <div className="modal-backdrop">
                        <motion.div className="modal-content" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                            ...
                            <p>Adding <strong>{raigadFortLocations[extensionModal.locationId]?.name}</strong> exceeds your remaining time budget.</p>
                    ...
                    {mapMode === 'illustrated' ? (
                        ...
                        <div className="v2-leaflet-wrapper">
                            <Suspense fallback={
                                ...
                                <RaigadLeafletMap
                                    optimizedPath={currentRoute ? currentRoute.route.map(id => ({ node: { ...raigadFortLocations[id], id } })) : []}
                                    simState={{ isSimulating, simStep, simPhase, simSpeed }}
                                />
                            </Suspense>
                        </div>
                    )}
                    <AnimatePresence>{isSimulating && currentRoute && (<SimulationOverlay phase={simPhase} step={simStep} route={currentRoute} locations={raigadFortLocations} speed={simSpeed} setSpeed={setSimSpeed} setStep={setSimStep} stop={handleSimulationComplete} />)}</AnimatePresence>
                        ...
                        <>
                            ...
                            <div className="glass-panel timeline">
                                {currentRoute.route.map((id, idx) => {
                                    const loc = raigadFortLocations[id];
...
const LocationIntelligencePanel = ({ locationId, route, isForced, weights, onClose, onAddToRoute, onReroute, interactionMode }) => {
    const loc = raigadFortLocations[locationId];
    useEffect(() => {
