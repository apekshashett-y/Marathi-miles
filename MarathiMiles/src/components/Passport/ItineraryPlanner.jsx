...
const TIME_POOL = { "1 Hour": 60, "2–3 Hours": 150, "Half Day": 240, "Full Day": 480 };
const FORT_ENTRY = 50; // ₹
                    ...
                    <div className="itin-result-grid">
                        ...
                        <div className="itin-map-col">
                            <div className="itin-map-card">
                                <div className="itin-map-header">🗺️ Optimised Route Map</div>
                                <MapContainer
                                ...
                                >
                                    ...
                                    {itinerary.stops.map((stop, i) => (
                                        <Marker key={stop.id} position={[stop.lat, stop.lng]}
                                            icon={TYPE_ICON[stop.type] || TYPE_ICON.culture}>
                                            <Popup>
                                                <div className="itin-popup">
                                                    <strong>{stop.icon} {stop.name}</strong>
                                                    <span>Stop {i + 1} · {fmtMin(stop.startMin)}</span>
                                                    <span>{stop.duration} min · {stop.costMin > 0 ? `₹${stop.costMin}+` : "Free"}</span>
                                                </div>
                                            </Popup>
                                        </Marker>
                                    ))}
                                    ...
                                    {mapPositions && mapPositions.length > 1 && (
                                        <Polyline
                                            ...
                                            pathOptions={{ color: "#c0622a", weight: 3, opacity: 0.85, dashArray: "10, 8" }}
                                        />
                                    )}
                                ...
                                <div className="itin-map-legend">
                                    {[
                                        ["culture", "#cc2200", "Culture / Fort"],
                                        ["food", "#e67e22", "Food Stop"],
                                        ["shopping", "#d4a017", "Shopping"],
                                        ["scenic", "#27ae60", "Scenic / Relax"],
                                ...
                                <div className="itin-route-stats">
                                    <div className="r-stat">📍 <strong>{itinerary.stops.length}</strong> stops</div>
                                    <div className="r-stat">🛣️ <strong>{totalRouteKm} km</strong></div>
                                    <div className="r-stat">⏱️ ~<strong>{Math.ceil(totalRouteKm / 30 * 60)} min</strong> driving</div>
                                </div>
                            </div>
                        </div>
                    </div>
